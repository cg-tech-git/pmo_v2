...
import { ChevronDown, Bell01, CheckCircle, X } from "@/utils/icons";
...
import { UserAvatarDropdown } from "@/components/marketing/header-navigation/user-avatar-dropdown";
...
export const Header = ({ items = headerNavItems, isFullWidth, isFloating, className, showToast, toastMessage, toastType }: HeaderProps) => {
    ...
    return (
        <header
            ref={headerRef}
            className={cx(
                "sticky top-0 z-50 flex h-18 w-full items-center justify-center bg-primary border-b border-gray-200 md:h-20",
                isFloating && "h-16 md:h-19 md:pt-3",
                isFullWidth && !isFloating ? "has-aria-expanded:bg-primary" : "max-md:has-aria-expanded:bg-primary",
                className,
            )}
        >
            <div className="flex size-full w-full flex-1 items-center px-4 md:px-16 bg-secondary">
                <div
                    className={cx(
                ...
                >
                    <div className="flex flex-1 items-center gap-5 md:pl-6">
                        <h1 className="text-lg">
                            <span className="text-gray-900 font-semibold">Al Laith Projects Services</span>
                            <span className="text-gray-500 font-normal">: Site Accreditation</span>
                        </h1>
                    ...
                    <div className="hidden items-center gap-3 md:flex">
                        ...
                        <button aria-label="Notifications" className="rounded-full p-2 hover:bg-primary_hover focus:outline-none">
                            <Bell01 className={`h-6 w-6 transition-colors duration-300 ${
                                showToast 
                                    ? `animate-jiggle ${
                                        toastType === 'success' ? 'text-green-600' : 
                                        toastType === 'error' ? 'text-red-600' : 
