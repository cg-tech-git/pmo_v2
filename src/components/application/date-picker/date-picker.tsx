...
export const DatePicker = ({ value: valueProp, defaultValue, onChange, onApply, onCancel, ...props }: DatePickerProps) => {
    ...
    return (
        <AriaDatePicker shouldCloseOnSelect={false} {...props} value={value} onChange={setValue}>
            <AriaGroup>
                <Button size="sm" color="secondary" iconLeading={CalendarIcon} className="rounded-lg before:rounded-[7px] text-tertiary font-normal h-[36px]">
                    {formattedDate}
                </Button>
            </AriaGroup>
            <AriaPopover
                        ...
                        <>
                            ...
                            <div className="grid grid-cols-2 gap-3 border-t border-secondary p-4">
                                ...
                                <Button
                                    size="md"
                                    color="primary"
                                     className="ring-[#56B3E5] hover:ring-[#49A3D2]"
                                    onClick={() => {
                                        onApply?.();
                                        close();
                                    }}
