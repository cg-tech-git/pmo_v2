...
import { LifeBuoy01, LogOut01, Settings01 } from "@/utils/icons";
...
export const SidebarNavigationSlim = ({ activeUrl, items, footerItems = [], hideBorder, hideRightBorder }: SidebarNavigationSlimProps) => {
    ...
    const mainSidebar = (
        <aside
            ...
            className="group flex h-full max-h-full max-w-full overflow-y-auto bg-gray-50 py-1 pl-1"
        >
            <div
                className={cx(
                    "flex w-auto flex-col justify-between rounded-xl pt-5 ring-1 ring-transparent transition duration-300 ring-inset",
                )}
            >
                <div className="flex justify-center px-3">
                    <ProjectLogo className="h-8 w-auto" />
                </div>
        ...
        <AnimatePresence initial={false}>
            {isSecondarySidebarVisible && (
                <motion.div
                    ...
                    className={cx(
                        "relative h-full overflow-x-hidden overflow-y-auto bg-gray-50",
                    )}
                >
                    <div style={{ width: SECONDARY_SIDEBAR_WIDTH }} className="flex h-full flex-col px-3 pt-5">
                        <div className="h-8 mb-4"></div>
                        <ul className="mt-4 flex flex-col gap-0.5">
                            {items.map((item) => (
                                <li key={item.label}>
                                    <NavItemBase current={activeUrl === item.href} href={item.href} badge={item.badge} type="link">
